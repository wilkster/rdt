      ...
      foreach {folder} $folders {
         ...
         foreach iconFile [lsort [glob -nocomplain -directory $folder "*.ico"]] {
            ...
            try {
               ...
               logerr $result
               winTaskBar::addIcon $icon $extToImage(rdt)
            }
            ...
            # Don't duplicate if exists (i.e. if user has an override)
            if {![info exists typeToIcon($icon)]} {
               try {
                  set Icon [winico create $iconFile]
                  set typeToIco($icon) $Icon
               } on error result {
                  logerr $result
               }
            }
            # restore state from settings
            set tbIconState($icon)  [S::getIconState $icon]

            set iconDef($Icon) "Recent $icon files\nLeft Button for list\nRight Button search"
            # Add the task bar icons (if enabled from settings)
            winTaskBar::TrayToggle $icon tbIconState($icon)
...
proc winTaskBar::iconExists {ext} {
   variable extToImage
   if {![info exists extToImage($ext)] || $extToImage($ext) eq ""} {
      return 0
   } else {
      return 1
   }
...
#       lassign $hashSet name sdate mdate mounted hits
#       db::setVal $name IconType "folder"
#       db::setVal $name Image $extToImage(folder)
#    }
...
#       # (below code should be in a function that is also used by hashShortcut
#       # like: winTaskBar::getAnIcon, called from hashShortcut
#       if {[winTaskBar::typeExists $ext]} {
...
#          set imgx [getShellIcon $name]
#          set extToImage($ext) $imgx
#          db::setVal $name Image $imgx
#       }  else {
#          db::setVal $name Image $extToImage($ext)
#       }
#    }
# }
...
proc winTaskBar::buildSettingMenu {Menu} {
   ...
   $Menu add cascade  -label "Limit Entries to..." -menu $Menu.limitNum \
      -compound left -image $extToImage(limit)
   $Menu add cascade  -label "Remove Entries older than (days)..." -menu $Menu.age \
      -compound left -image $extToImage(clock)
   ...
   $Menu add cascade  -label "Include File Types..." -menu $Menu.type \
     -compound left  -image $extToImage(filter)

   #dynamically turn on/off icons rather than require a restart
   foreach {icon} [array names tbIconState] {
      if {[winTaskBar::iconExists $icon]} {
         $Menu.type add checkbutton -compound left -image $extToImage($icon) \
            -label $icon -variable winTaskBar::tbIconState($icon) \
            -command [list winTaskBar::TrayToggle $icon winTaskBar::tbIconState($icon)]
      } else {
         $Menu.type add checkbutton -label $icon -variable winTaskBar::tbIconState($icon) \
            -command [list winTaskBar::TrayToggle $icon winTaskBar::tbIconState($icon)]
      }
   }
   $Menu add separator
   $Menu add command -label "About" -command "rdt::About" \
      -compound left -image $extToImage(about)

   $Menu add command -label "Exit RDT" -command "rdt::unload" \
      -compound left -image $extToImage(exit)
   return $Menu
...
#puts "postMenu $icon"
 ...
 #     buildSettingMenu $Menu
      ...
      $Menu add cascade -label "Settings" -menu $Menu.settings \
         -compound left -image $winTaskBar::extToImage(settings)
      ...
      if {$isFolder} {
         $Menu add command \
            ...
            -compound left -image $extToImage(search)
      } else {
         $Menu add command \
            -label "Search Dialog" \
...
#            set label "[clock format $fdate -format {%D %H:%M:%S}]    $file   ($folder)"
   ...
   if {$pinned} {
      $Menu add command \
         ...
         -compound left -image $extToImage(pinned) \
